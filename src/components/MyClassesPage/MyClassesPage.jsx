...
// ^^^^^^^^^^^^DELETE this when we have our classes reducer running^^^^^^^^^^^^^^^^^
  ...
  const myClasses = useSelector(store => store.userClass)
  ...
  return (
    <div>
      ...
      <ul>
      {myClasses.map((classEvent, i) =>( // "classEvent" was formerly called "event", look out for any residual errors from this.
